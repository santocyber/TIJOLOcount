...
import { BrickCalculator } from "./calculator.js";
import { Wall3DBuilder } from "./model_3d.js";
...
function showModel(modelGroup, keepCamera = false) {
    ...
    if (raw) {
      ...
      if (data.layers && data.layers.length > 0) {
        ...
        if (data.mortar_joint)
          document.getElementById("mortar-joint-input").value = (data.mortar_joint * 100).toFixed(1);
        if (data.snap_size) {
        ...
        if (data.endpoint_snap !== undefined) {
          ...
          document.getElementById("endpoint-snap-toggle").checked = data.endpoint_snap;
        }
...
function autoSave() {
  try {
    ...
    const data = {
      ...
      mortar_joint: parseFloat(document.getElementById("mortar-joint-input").value || "1.0") / 100,
...
function newLayer() {
  saveCurrentWalls();
  const prev = layers.length > 0
    ? layers[layers.length - 1].elevation + (layers[layers.length - 1].height || 2.8)
    : 0;
...
const modalOnProgress = (cur, total) => {
  document.getElementById("modal-progress-fill").style.width = ((cur / total) * 100).toFixed(0) + "%";
  document.getElementById("modal-progress-text").textContent =
    cur.toLocaleString("pt-BR") + " / " + total.toLocaleString("pt-BR") + " tijolos";
};
...
function openModal() {
  ...
  const sunTarget = parseFloat(document.getElementById("sun-preset-select").value);
...
document.getElementById("speed-select").addEventListener("change", (e) => {
  if (modalAnimator) {
    ...
    modalAnimator._brickDuration = Math.max(1, modalAnimator.totalBricks / (modalAnimator.bricksPerFrame * 60));
  }
});
...
document.getElementById("sun-preset-select").addEventListener("change", (e) => {
...
function activateCutout(type) {
  ...
  const w = parseFloat(document.getElementById(type === "door" ? "door-size-select" : "window-size-select").value);
  ...
  cutout3d.activate();
  document.getElementById("status-bar").textContent =
    "Modo corte 3D: " + (type === "door" ? "Porta" : "Janela") + " - clique na parede";
}
...
document.getElementById("btn-zoom-in").addEventListener("click", () => { floorplan.zoomIn(); });
document.getElementById("btn-zoom-out").addEventListener("click", () => { floorplan.zoomOut(); });
...
document.getElementById("door-size-select").addEventListener("change", (e) => {
  if (cutoutActive === "door") {
    cutout3d.setConfig({ width: parseFloat(e.target.value), height: 2.1, elevation: 0 });
    cutout3d.setWallData(allLayersData(), calcWallThickness());
  }
});
document.getElementById("window-size-select").addEventListener("change", (e) => {
  if (cutoutActive === "window") {
    cutout3d.setConfig({ width: parseFloat(e.target.value), height: 1.0, elevation: 1.1 });
    cutout3d.setWallData(allLayersData(), calcWallThickness());
  }
});
...
document.getElementById("endpoint-snap-toggle").addEventListener("change", (e) => {
...
async function doGenerate() {
  ...
  const brickType = document.getElementById("brick-type-select").value;
  const mortarJoint = parseFloat(document.getElementById("mortar-joint-input").value || "1.0") / 100;
  const orientation = document.getElementById("orientation-select").value;

  try {
    const calc = new BrickCalculator({
      layers: allLayers,
      brick_type_key: brickType,
      mortar_joint: mortarJoint,
      orientation,
    });

    if (calc.totalWallArea <= 0) {
      throw new Error("Area total das paredes invalida.");
    }

    const data = calc.summary;

    document.getElementById("res-total-area").textContent = data.area_total_paredes_m2 + " m\u00b2";
    document.getElementById("res-cement").textContent =
      (data.cimento_kg || 0).toLocaleString("pt-BR", { maximumFractionDigits: 1 }) + " kg";
    document.getElementById("res-sand").textContent =
      (data.areia_kg || 0).toLocaleString("pt-BR", { maximumFractionDigits: 1 }) + " kg";
    document.getElementById("res-water").textContent =
      (data.agua_l || 0).toLocaleString("pt-BR", { maximumFractionDigits: 1 }) + " L";
    document.getElementById("res-bpm2").textContent = data.tijolos_por_m2 + " un/m\u00b2";
    document.getElementById("res-total-bricks").textContent =
      data.total_tijolos.toLocaleString("pt-BR");
    resultsBar.classList.add("show");

    const builder = new Wall3DBuilder(calc);
    const modelGroup = builder.build();

    document.getElementById("viewer-placeholder").style.display = "none";
    showModel(modelGroup, cutoutActive !== null);

    const glbBlob = await builder.exportGLB();
    document.getElementById("btn-download-gltf").href = URL.createObjectURL(glbBlob);
    const stlBlob = builder.exportSTL();
    document.getElementById("btn-download-stl").href = URL.createObjectURL(stlBlob);

    lastBrickPositions = calc.allBrickPositions;
    lastBrickDims = calc.effectiveBrickDims;
    btnAnimate.style.display = lastBrickPositions.length > 0 ? "inline-block" : "none";
...
["brick-type-select", "mortar-joint-input", "orientation-select"].forEach((id) => {
